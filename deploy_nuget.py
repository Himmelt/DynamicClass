...
async def build_test_pack_and_publish(version: str) -> int:
    ...
    # 4. 构建项目
    print("4. 构建项目...")
    if await run_command("dotnet build DynamicClass/DynamicClass.csproj --configuration Release") != 0:
        print("构建失败!")
        return 1

    # 5. 打包项目
    print("5. 打包项目...")
    if await run_command("dotnet pack DynamicClass/DynamicClass.csproj --configuration Release --output nupkg --no-build") != 0:
        print("打包失败!")
        return 1
...
async def main():
    """主函数"""
    # 获取用户输入的版本号和版本描述
    print("=== DynamicClass NuGet 包发布工具 ===")

    # 先读取当前版本号
    script_dir = os.path.dirname(os.path.abspath(__file__))
    csproj_path = os.path.join(script_dir, "DynamicClass/DynamicClass.csproj")
    ...
    package_file_name = f"Himmelt.DynamicClass.{version}.nupkg"
    symbol_package_file_name = f"Himmelt.DynamicClass.{version}.snupkg"
